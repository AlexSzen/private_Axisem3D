            ...
            for (int alpha = 0; alpha < Nr; alpha++) {
                ...
                for (const auto &model: m3D) {
                    ...
                    for (int iprop = 0; iprop < propertiesTIso.size(); iprop++) {
						...
						double perturb = 1.; // SMALL PERTURBATION FOR TEST KERNELS
						...
						//	if (properties[iprop] == Volumetric3D::MaterialProperty::VP) { //only perturb VP for misfit tests
                            	mat3D(alpha, ipnt) = ref1D * (1. + value3D);
						//	} else {
						//		mat3D(alpha, ipnt) = ref1D;
						//	}
