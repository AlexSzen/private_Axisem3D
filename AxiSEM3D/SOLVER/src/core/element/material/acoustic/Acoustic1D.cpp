...
void Acoustic1D::potToDisp(FluidResponse &response) const {
    for (int alpha = 0; alpha <= response.mNu; alpha++) {
        const ar3_CMatPP &strain_alpha = response.mStrain[alpha];
        ar3_CMatPP &stress_alpha = response.mStress[alpha];
        stress_alpha[0] = mKStructNoQuad.schur(strain_alpha[0]);
        stress_alpha[1] = mKStructNoQuad.schur(strain_alpha[1]);
        stress_alpha[2] = mKStructNoQuad.schur(strain_alpha[2]);
    }
}
