...
const vec_CMatPP& SolidElement::getPwave() const { //temporary implementation for P wave output 
	...
	for (int inu = 0; inu <= sResponse.mNu; inu++) {
		sResponse.mPwave[inu] = sResponse.mStrain9[inu][0] + sResponse.mStrain9[inu][4] + sResponse.mStrain9[inu][8];
	}
