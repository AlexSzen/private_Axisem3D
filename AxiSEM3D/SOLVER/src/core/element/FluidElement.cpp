...
const vec_CMatPP &FluidElement::getPwave() const { //temporary implementation to output div(grad(u))
	...
	if (mInTIso) {
 	...
 	mGradient->computeGrad9(sResponse.mStress, sResponse.mRStrain, sResponse.mNu, sResponse.mNyquist);
	
	for (int inu = 0; inu <= sResponse.mNu; inu ++) {
		sResponse.mPwave[inu] = sResponse.mRStrain[inu][0] + sResponse.mRStrain[inu][4] + sResponse.mRStrain[inu][8];
	}
	
	return sResponse.mPwave;
...
void FluidElement::initWorkspace(int maxMaxNu) {
	sResponse.mPwave = vec_CMatPP(maxMaxNu + 1, CMatPP::Zero());
	sResponse.mRStrain = vec_ar9_CMatPP(maxMaxNu + 1, zero_ar9_CMatPP);
