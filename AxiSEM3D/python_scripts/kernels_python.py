...
wc_fwd = WavefieldComputer(wavefield,nu,nu,s,z,sem_mesh)
wc_adj = WavefieldComputer(kernels,nuKer,nuKer,s,z,sem_mesh)
wc_rho = WavefieldComputer(rho,nu,nu,s,z,sem_mesh)
wc_vp = WavefieldComputer(vp,nu,nu,s,z,sem_mesh)
...
for i_slice in range(SLICES):
    x_slice, y_slice, z_slice, wvf_slice_fwd = wc_fwd.compute_slice(RMIN[i_slice], RMAX[i_slice], PHIS_SLICES[i_slice], COMP_SLICES[i_slice])
    x_slice_adj, y_slice_adj, z_slice_adj, wvf_slice_adj = wc_adj.compute_slice(RMIN[i_slice], RMAX[i_slice], PHIS_SLICES[i_slice], COMP_SLICES[i_slice])
    x_slice_rho, y_slice_rho, z_slice_rho, rho_slice = wc_rho.compute_slice(RMIN[i_slice], RMAX[i_slice], PHIS_SLICES[i_slice], COMP_SLICES[i_slice])
    x_slice_vp, y_slice_vp, z_slice_vp, vp_slice = wc_vp.compute_slice(RMIN[i_slice], RMAX[i_slice], PHIS_SLICES[i_slice], COMP_SLICES[i_slice])
    points_slice = list(zip(x_slice,y_slice,z_slice))
    range_slice = range(len(x_slice))
    ker_tot = np.zeros(len(x_slice), dtype=np.float32)
    for it in range(num_steps):
        ker_slice1 = 2*dt*np.multiply(wvf_slice_fwd[it],wvf_slice_adj[it])
        ker_slice2 = np.multiply(ker_slice1,rho_slice[0])
        ker_slice = np.multiply(ker_slice2,vp_slice[0])
        ker_tot += ker_slice1
        vtk = pyvtk.VtkData(
                pyvtk.UnstructuredGrid(points_slice,range_slice),
                pyvtk.PointData(pyvtk.Scalars(ker_tot,name='wavefield_slice')),
                'animation'
                )
